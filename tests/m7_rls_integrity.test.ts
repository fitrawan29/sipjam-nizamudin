 ...
 *
 * This test suite provides authentic, hostile adversarial, and uncheatable
 * verification for Supabase Row Level Security (RLS) policies and Multi-Tenant Isolation:
 *
 ...
 *
 * 4. HOSTILE ADVERSARIAL ATTACKS & PRIVILEGE ESCALATION DEFENSE:
 *    - Unauthenticated client sending ONLY {'x-user-role': 'Superadmin'} without
 *      x-user-id receives 0 rows from users and cannot mutate tables.
 *    - Forged non-existent x-user-id with Superadmin role receives 0 rows.
 *    - School Admin x-user-id cannot escalate to Superadmin (u.sekolah_id IS NOT NULL).
 *    - School Admin with x-sekolah-id cannot mutate platform tables or delete other schools.
 *    - Cross-tenant user provisioning is strictly blocked.
 * ============================================================================
 */
...
async function runRlsIntegrityVerification() {
  console.log(`\n${CYAN}${BOLD}======================================================================${RESET}`);
  console.log(`${CYAN}${BOLD}     M7/M8 ADVERSARIAL RLS INTEGRITY & MULTI-TENANT VERIFICATION SUITE ${RESET}`);
  ...
  // 5. Spoofed School Admin client (School A Admin attempting to claim Superadmin with school scope)
