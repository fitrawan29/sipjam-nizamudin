...
--      and prevent unauthenticated x-user-role: Superadmin header spoofing.
--    - Verify is_superadmin() references hardened role resolution.
...
--    - Remove permissive 'OR true' from sekolah_select_policy.
...
--    - Remove permissive 'OR true' from users_select_policy.
--    - Strict SELECT: is_superadmin() OR (get_auth_user_sekolah_id() IS NOT NULL AND sekolah_id = get_auth_user_sekolah_id()).
--    - Maintain strict INSERT/UPDATE/DELETE policies for Superadmin and School Admin.
--
-- 4. REMEDIATE 16 TENANT TABLES:
--    - Drop flawed policies with 'OR (public.get_auth_user_sekolah_id() IS NULL AND true)'
