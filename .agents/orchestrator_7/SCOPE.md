...
|---|------|-------|-------------|--------|
| M7.1 | Multi-Tenant Database & RLS Migration | SQL migration, table alterations, backfill, unique constraints, RLS policies, TypeScript types | none | IN_PROGRESS |
| M7.2 | Superadmin & Admin Hierarchy | `SuperadminView`, `/superadmin` page, Admin creation modal, dynamic header, navigation isolation | M7.1 | PLANNED |
| M7.3 | App Tenant Context & Scoping | Update `AdminDataView`, `AdminConfigView`, `AdminBackupView`, `PrintHeader` for `sekolah_id` | M7.1, M7.2 | PLANNED |
| M7.4 | Ascending Date Sorting | Fix sorting in `RekapJurnalView`, `RekapSiswaView`, `AdminRekapView`, `PiketView` | M7.1 | PLANNED |
