import TPMatrixTab from './gradebook/TPMatrixTab';
import RekapSemesterTab from './gradebook/RekapSemesterTab';
import StatistikTab from './gradebook/StatistikTab';
'use client';
...
export default function GradebookView({ user }: GradebookViewProps) {
  ...
  const handleSaveGrades = async () => {
    ...
    if (dirtyKeys.length === 0) {
      Swal.fire('Informasi', 'Tidak ada perubahan nilai yang perlu disimpan.', 'info');
      return;
    }
    ...
    try {
      ...
      Swal.fire({
        ...
        text: `${recordsToUpsert.length} entri nilai telah tersimpan di database.`,
        timer: 1800,
        showConfirmButton: false,
      });
    } catch (err: any) {
      console.error('Error saving grades:', err);
      Swal.fire('Gagal Menyimpan', err.message || 'Terjadi kesalahan saat menyimpan nilai.', 'error');
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const handleSaveTp = async (e: React.FormEvent) => {
    ...
    try {
      if (editingTp) {
        ...
        Swal.fire({
          icon: 'success',
          title: 'TP Berhasil Dibuat',
          text: 'Tujuan Pembelajaran dan kolom penilaian awal telah disiapkan.',
          timer: 1500,
          showConfirmButton: false,
        });
      }
      ...
      Swal.fire('Gagal Menyimpan TP', err.message || 'Terjadi kesalahan sistem.', 'error');
    }
  };
  ...
  const handleDeleteTp = async (tp: TujuanPembelajaran) => {
    ...
    try {
      ...
      Swal.fire('Terhapus', 'Tujuan Pembelajaran telah dihapus.', 'success');
      await fetchTPs();
    } catch (err: any) {
      console.error('Error deleting TP:', err);
      Swal.fire('Gagal Menghapus TP', err.message || 'Terjadi kesalahan.', 'error');
    }
  };
  ...
  const handleSaveCol = async (e: React.FormEvent) => {
    ...
    try {
      if (editingCol) {
        ...
        if (error) throw error;
        Swal.fire('Berhasil', 'Kolom asesmen telah diperbarui.', 'success');
      } else {
        ...
        if (error) throw error;
        Swal.fire('Berhasil', `Kolom ${colForm.kategori} baru berhasil ditambahkan.`, 'success');
      }
      ...
      Swal.fire('Gagal Menyimpan Kolom', err.message || 'Terjadi kesalahan.', 'error');
    }
  };
  ...
  const handleDeleteCol = async (col: AsesmenKolom) => {
    ...
    try {
      ...
      Swal.fire('Terhapus', `Kolom "${col.nama}" berhasil dihapus.`, 'success');
      if (selectedTpId) await fetchColumnsForActiveTP(selectedTpId);
      await fetchGrades();
    } catch (err: any) {
      console.error('Error deleting column:', err);
      Swal.fire('Gagal Menghapus Kolom', err.message || 'Terjadi kesalahan.', 'error');
    }
  };
  ...
  const handleExecuteBulkFill = (e: React.FormEvent) => {
    ...
    Swal.fire({
      icon: 'success',
      title: 'Pengisian Cepat Berhasil',
      text: `${filledCount} siswa telah diisi dengan nilai ${val}. Jangan lupa klik 'Simpan Semua Nilai'.`,
      timer: 2000,
      showConfirmButton: false,
    });
  };
  ...
  return (
    <section className="space-y-5 pb-12">
      ...
      {/* ========================================================================= */}
      {activeTab === 'tp-matrix' && <TPMatrixTab {...tabProps} />}

      {/* ========================================================================= */}
      {/* TAB 2: REKAP NILAI RAPOR SEMESTER (OVERVIEW ALL TPS)                      */}
      {/* ========================================================================= */}
      {activeTab === 'rekap-semester' && <RekapSemesterTab {...tabProps} />}

      {/* ========================================================================= */}
      {/* TAB 3: STATISTIK & ANALISIS KELAS                                         */}
      {/* ========================================================================= */}
      {activeTab === 'statistik' && <StatistikTab {...tabProps} />}
