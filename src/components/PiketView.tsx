              ...
              <div id="piket-content-beranda" className="space-y-4 fade-in">
                  <div className="bg-teal-50 dark:bg-teal-900/10 border border-teal-100 dark:border-teal-900/50 p-4 rounded-2xl">
                      ...
                      <div className="space-y-2 max-h-40 overflow-y-auto custom-scroll pr-1">
                        {jadwalPiket.length === 0 ? (
                          <div className="text-center text-[10px] text-gray-500 dark:text-gray-400 py-2">Belum ada jadwal.</div>
                        ) : jadwalPiket.map(j => (
                  ...
                  <div>
                      <div className="flex justify-between items-center mb-3 px-1">
                          <h3 className="text-xs font-bold text-gray-700 dark:text-gray-300"><i className="fa-solid fa-list-check mr-1 text-gray-400 dark:text-gray-500"></i> Laporan Terbaru</h3>
                      </div>
                      <div className="space-y-3 min-h-[150px]">
                        {laporanPiket.length === 0 ? (
                          <div className="text-center text-[10px] text-gray-500 dark:text-gray-400 py-4">Belum ada laporan.</div>
                        ) : laporanPiket.map(l => (
                          <div key={l.id} className="bg-white dark:bg-gray-800 p-3 rounded-xl border border-gray-100 dark:border-gray-700 shadow-sm">
                            <div className="flex justify-between items-start mb-1">
                              ...
                              <div className="text-[9px] text-gray-400 dark:text-gray-500">{l.tanggal}</div>
                            </div>
            ...
            {activeTab === 'lapor' && canReport && (
              <div id="piket-content-form" className="fade-in space-y-4">
                  ...
                  <form onSubmit={handlePiketSubmit} className="space-y-4">
                      <div>
                        <label className="block text-[11px] font-bold text-gray-500 dark:text-gray-400 mb-1.5 ml-1">Tanggal Piket</label>
                        <input type="date" required value={getWitaDateStr()} readOnly className="w-full px-3 py-2.5 text-sm rounded-xl input-premium bg-gray-100 dark:bg-gray-800 dark:text-white cursor-not-allowed" />
                      </div>
                      ...
                      <div>
                        <label className="block text-[11px] font-bold text-gray-500 dark:text-gray-400 mb-1.5 ml-1">Catatan Khusus</label>
                      ...
                      <div>
                        <label className="block text-[11px] font-bold text-gray-500 dark:text-gray-400 mb-1.5 ml-1">Upload Foto Dokumentasi Piket <span className="text-red-500 dark:text-red-400">(Wajib)</span></label>
                        <input type="file" accept="image/*" onChange={e => setFile(e.target.files ? e.target.files[0] : null)} required className="w-full px-3 py-2 text-sm rounded-xl input-premium bg-white dark:bg-gray-800 dark:text-white" />
                      </div>
                      <div className="pt-2">
